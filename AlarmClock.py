...
def main_window():
    ...
    welcome = Label(window, text="AlarmClock")
...
def setting_alarm():
    ...
    if int(get_minute) < 10 and int(get_hour) < 10:
        message = Label(window, text="Votre alarme est prévue pour 0{0}h0{1}min".format(get_hour, get_minute),
                        font=('Bahnschrift', 15, 'bold'), fg='light sea green', bg="ivory")
        message.grid(row=7, column=1)
    elif int(get_minute) < 10:
        message = Label(window, text="Votre alarme est prévue pour {0}h0{1}min".format(get_hour, get_minute),
                        font=('Bahnschrift', 15, 'bold'), fg='light sea green', bg="ivory")
        message.grid(row=7, column=1)
    elif int(get_hour) < 10:
        message = Label(window, text="Votre alarme est prévue pour 0{0}h{1}min".format(get_hour, get_minute),
                        font=('Bahnschrift', 15, 'bold'), fg='light sea green', bg="ivory")
        message.grid(row=7, column=1)
    else:
        message = Label(window, text="Votre alarme est prévue pour {0}h{1}min".format(get_hour, get_minute),
                        font=('Bahnschrift', 15, 'bold'), fg='light sea green', bg="ivory")
        message.grid(row=7, column=1)
    def verification():
